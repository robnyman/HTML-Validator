	...
	validate = function (validation, html) {
		chrome.tabs.getSelected(null, function (tab) {
			...
			if (validatorType === "inline" && (!(/(validator\.nu|validator\.w3\.org)/.test(tabUrl)) && autorun)) { // To avoid inline validation of the W3C validator
				...
				var url = "http://validator.w3.org/check?uri=" + encodeURIComponent(tabUrl) + "&output=json",
				...
				if (usetimeout === "true") {
					timer = window.setTimeout(function () {
						xhr.abort();
						sendResults({
							"message" : "Validation timed out. Please try again or <a href=\"http://validator.w3.org/check?uri=" + encodeURIComponent(tabUrl) + "\" target=\"_blank\">validate this page at W3C</a>"
						});
					}, 5000);
				}
				...
				xhrInline.open("POST", "http://validator.w3.org/check", true);
				xhrInline.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
				xhrInline.send(params);
			}
			else if (validatorType === "new-tab" && !(/validator\.w3\.org/.test(tabUrl))) { // URL check to avoid validating the W3C validator... :-)
				chrome.tabs.create({
					url : "http://validator.w3.org/check?uri=" + encodeURIComponent(tabUrl)
				});
			}
